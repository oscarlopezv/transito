...
const agregarCampo = () =>
{
    inputs.value.push({label:"Archivo"});   
}

const eliminarCampo = (index) =>
{
    inputs.value.splice(index, 1);
    form.files.splice(flag,1);
}
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    <form class="max-w-7xl mx-5 p-4" @submit.prevent="form.post(route('ejecution.store'))" enctype="multipart/form-data">
                        ...
                        <div class="mb-5">
                            ...
                            <textarea id="" name="" v-model="form.description" class="block w-full p-4 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-base"></textarea>
                        </div>
                        ...
                        <div class="grid grid-cols-3 gap-6">
                            <div class="" v-for="(item,index) in inputs" :key="index" >
                                ...
                                <input type="file" @change="obtenerArchivos" id="name" name="name" class="block w-full p-4 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-base " >                                
                                <label for="" class="block text-sm font-medium text-red-950 text-right hover:cursor-pointer" @click="eliminarCampo(index)">[X]</label>
                            </div>
                        </div>
